            ...
            >
              <div className="relative">
                ...
                <div className="relative flex h-9 w-9 md:h-10 md:w-10 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-600 to-purple-600">
                  <span className="text-lg md:text-xl font-black text-white">P</span>
                </div>
              </div>
              ...
              <div className="flex flex-col space-y-1">
                {navLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="group flex items-center gap-3 rounded-lg px-4 py-3 text-white/90 transition-all hover:bg-white/10 hover:text-white"
...
function NavLink({ href, children, icon, className = "" }) {
  return (
    <Link
      href={href}
      className={`group relative flex items-center gap-2 rounded-lg px-4 py-2.5 font-medium text-white/80 transition-all hover:text-white ${className}`}
    >
      {icon && <span className="opacity-70 group-hover:opacity-100 transition-opacity">{icon}</span>}
      <span>{children}</span>
      <span className="absolute bottom-0 left-1/2 h-0.5 w-0 -translate-x-1/2 bg-gradient-to-r from-transparent via-indigo-500 to-transparent transition-all duration-300 group-hover:w-3/4"></span>
    </Link>
  );
}